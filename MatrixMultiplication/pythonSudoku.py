...
'''
pythonSudoku.py
Neural net using my matrix math operations, trains on and classifies / solves Sudokus
Justin Kahr
'''
    ...
    '''
        ...
        while(True):
            ...
            while(hid_layer < self.num_hid_layers):
                ...
                toHid[hid_layer] = mm.matrix_basis_add(mm.matrix_mult(hidden_vals[hid_layer-1], self.Whh[hid_layer-1]) , self.Bhh[hid_layer-1])
                #toHid[hid_layer] = mm.vector_dot(hidden_vals[hid_layer-1], mm.matrix_add(self.Whh[hid_layer-1] , self.Bhh[hid_layer-1]))
                hidden_vals[hid_layer] = self.tanh_af(toHid[hid_layer]).tolist()
                hid_layer += 1
            ...
            while(hid_layer > 0):
                ...
                deltaH[hid_layer-1] = mm.matrix_elem_mult(mm.matrix_mult(deltaH[hid_layer] , mm.matrix_transpose(self.Whh[hid_layer-1])) , self.tanh_af_deriv(hidden_vals[hid_layer - 1]).tolist())
                hid_layer -= 1
...
random weights
    ...
    '''
        ...
        self.Whh = [None]*(num_hid_layers-1)
        #Bhh is biases of hidden layer connections
        self.Bhh = [None]*(num_hid_layers-1)
...
#   momentum
#   random_seed
test_agent = NeuralMMAgent(81, 729, 18, 81, max_epoch=10)
# Sudoku training data
NUM_SUDOKU=1000
test_in = sr.get_sudoku_n(NUM_SUDOKU)
test_out = sr.get_solution_n(NUM_SUDOKU)
# Train the agent
