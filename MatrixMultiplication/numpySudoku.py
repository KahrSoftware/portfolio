...
'''
numpySudoku.py
Neural net using numpy matrix operations, trains on and classifies / solves Sudokus
Justin Kahr
'''

class NeuralMMAgent(object):

    def __init__(self, num_in_nodes, num_hid_nodes, num_hid_layers, num_out_nodes, learning_rate = 0.2, max_epoch=10000, max_sse=.01, momentum=0.4, activation_function=None, random_seed=1):
        '''
		      Arguments:
    ...
    def train_net(self, input_list, output_list, max_num_epoch=100000, max_sse=0.1):
        ...
        while(True):
            ...
            hidden_vals[0] = self.tanh_af(toHid[0])
            ...
            output_vals = self.sigmoid_af(toOut)
            ...
            sse = numpy.mean(error**2)
            all_err.append(sse)
            # Print status each epoch
            print("Epoch: "+str(epoch)+" | sse: "+str(sse))
            print("\ttime: "+str(time.time()-epochTime))
            epoch += 1
...
test_out = sr.get_sudoku_n(NUM_SUDOKU)
# Train the agent
